...
export class ValidationService {
  static getValidatorErrorMessage(validatorName: string, validatorValue?: any) {
    const config = {
      ...
      'invalidEmailAddress': 'Incorrect email format (abc@def.xyz).',
      'invalidPassword': 'Password must contain at least 6 characters and numbers.',
      'invalidNumber': 'You must enter the number.',
      'invalidPhoneNumber': 'You must enter the number.',
      'minlength': `Minimum length ${validatorValue.requiredLength}`,
      'ConfirmPassword': 'the two passwords must match!'
    };
